...
contract DAppMeeting {
    ...
    modifier participantExists(string memory _roomId) {
        require(
            participantsInRoom[_roomId][msg.sender],
    ...
    function removeAuthorizedBackend(address _backend) public {
        for (uint i = 0; i < authorizedBackends.length; i++) {
            if (authorizedBackends[i] == _backend) {
                ...
                authorizedBackends[i] = authorizedBackends[
                    authorizedBackends.length - 1
                ];
    ...
    function joinRoom(
        ...
        bytes memory sessionDescription
    ...
    function leaveRoom(
        string memory _roomId
    ) public roomExists(_roomId) participantExists(_roomId) {
        ...
        if (participantIndex < rooms[_roomId].participants.length - 1) {
            Participant memory lastParticipant = rooms[_roomId].participants[
                rooms[_roomId].participants.length - 1
            ];
            rooms[_roomId].participants[participantIndex] = lastParticipant;
            participantIndices[_roomId][
                lastParticipant.walletAddress
            ] = participantIndex;
        }
    ...
    function forceLeaveRoom(
        ...
        uint participantIndex = participantIndices[_roomId][_participant];
        if (
            participantIndex == 0 &&
            rooms[_roomId].participants[0].walletAddress != _participant
        ) {
            return;
        }
    ...
    function setParticipantSessionID(
        string memory _roomId,
        address _participantAddress,
        string memory _sessionID
    ) public roomExists(_roomId) {
        require(
            participantsInRoom[_roomId][_participantAddress],
            "Participant not in room"
        );

        uint participantIndex = participantIndices[_roomId][
            _participantAddress
        ];
        ...
        for (
            uint i = 0;
            i < participantTracks[_roomId][_participantAddress].length;
            i++
        ) {
            participantTracks[_roomId][_participantAddress][i]
    ...
    function forwardEventToBackend(
        string memory _roomId,
    ...
    function forwardEventToFrontend(
        string memory _roomId,
        address _participant,
        bytes memory _eventData
    ) public roomExists(_roomId) {
        require(
            participantsInRoom[_roomId][_participant],
            "Target participant not in room"
        );
    ...
    function getParticipantTracks(
        string memory _roomId,
        address _participant
    ) public view roomExists(_roomId) returns (Track[] memory) {
        require(
            participantsInRoom[_roomId][_participant],
            "Participant not in room"
        );
    ...
    function getRoomParticipantsCount(
        string memory _roomId
    ) public view roomExists(_roomId) returns (uint) {
        return rooms[_roomId].participants.length;
    }

    function getParticipantTracksCount(
        string memory _roomId,
        address _participant
    ) public view roomExists(_roomId) returns (uint) {
        require(
